...
(async () => {
    //task 1
    //queryInterface.addColumn('product_categories', 'shop_id', { type: DataTypes.INTEGER });
    ...
    //     let date = new Date(product['scraped_at']);
    //     const time_dim = await Time_dim.findOne({ where: { date_actual: date } });
    //     console.log(time_dim["date_dim_id"]);
    //     break;
    //     //let currentPageData = await productCategories(product[obj]);
    // }
